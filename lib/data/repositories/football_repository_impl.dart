...
class FootballRepositoryImpl implements FootballRepository {
  ...
  @override
  Future<Either<Failure, Match>> getMatchDetails(int matchId) async {
    if (await networkInfo.isConnected) {
      try {
  ...
  Future<Either<Failure, Player>> getPlayerInformation(int playerId) async {
    if (await networkInfo.isConnected) {
      try {
        final remotePlayer =
            await remoteDataSource.getPlayerInformation(playerId);
        ...
        try {
          final localPlayer =
              await localDataSource.getCachedPlayerInformation(playerId);
