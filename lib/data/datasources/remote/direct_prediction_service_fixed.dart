...
// Import shared models with a namespace to avoid conflicts
...
class DirectPredictionService {
      ...
      try {
        ...
        if (responseBody['response'] is List &&
          ...
          if (predictionItem is Map<String, dynamic>) {
            ...
            // Create percent object for the predictions
            final percent = Percent(
                home: percentData['home']?.toString() ?? '33%',
                draw: percentData['draw']?.toString() ?? '33%',
                away: percentData['away']?.toString() ?? '33%');

            // Extract goals data or use null if not available
            PredictionGoals? goals = null;
            if (predictionsData['goals'] is Map<String, dynamic>) {
              final goalsData =
                  predictionsData['goals'] as Map<String, dynamic>;
              goals = PredictionGoals(
                  home: goalsData['home'] is int ? goalsData['home'] : null,
                  away: goalsData['away'] is int ? goalsData['away'] : null);
            }
            ...
            // Determine if under/over is present
            final dynamic underOver = predictionsData['under_over'];

            // Create a PredictionData object with our transformed data
            return PredictionData(
              predictions: Predictions(
                winner: winner,
                winOrDraw: false, // Default value as it's required in the model
                underOver: underOver,
                goals: goals,
                advice: advice,
                percent: percent,
  ...
  // Helper methods to extract components from the JSON response using shared model classes
  shared_models.League _extractLeague(Map<String, dynamic>? json) {
    if (json == null) {
      return shared_models.League(
        id: 0,
        name: 'Unknown League',
        country: 'Unknown',
        logo: '',
  ...
  shared_models.Fixture _extractFixture(Map<String, dynamic>? json) {
    if (json == null) {
      return shared_models.Fixture(
        id: 0,
        timezone: 'UTC',
        date: '',
        timestamp: 0,
        venue: shared_models.FixtureVenue(name: 'Unknown'),
        status: shared_models.FixtureStatus(long: 'Not Started', short: 'NS'),
      );
    }
    ...
    return shared_models.Fixture(
      ...
      venue: shared_models.FixtureVenue(
        name: json['venue']?['name'] ?? 'Unknown',
        city: json['venue']?['city'],
      ),
      status: shared_models.FixtureStatus(
  ...
  shared_models.Teams _extractTeams(Map<String, dynamic>? json) {
    if (json == null) {
      return shared_models.Teams(
        home: shared_models.Team(id: 0, name: 'Home Team', logo: ''),
        away: shared_models.Team(id: 0, name: 'Away Team', logo: ''),
      );
    }

    return shared_models.Teams(
      home: shared_models.Team(
        id: json['home']?['id'] ?? 0,
        name: json['home']?['name'] ?? 'Home Team',
        logo: json['home']?['logo'] ?? '',
      ),
      away: shared_models.Team(
        id: json['away']?['id'] ?? 0,
